  ...
  .forEach(function (p) {
    ...
    if (args[thisPos - 2].type === 'Literal') {
      p.value.arguments = args.slice(0, thisPos - 2).concat([
        j.objectExpression([
          j.objectProperty(
            utils.isValidIdentifierNameForProperty(args[thisPos - 2].value) ?
              j.identifier(args[thisPos - 2].value) : j.literal(args[thisPos - 2].value),
            j.identifier(args[thisPos - 1].name)
          )
        ])
      ]);
    } else {
